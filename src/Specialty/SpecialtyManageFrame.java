...
public class SpecialtyManageFrame extends JFrame {
	...
	Object[][] arrData = {};
	String[] arrField = { "编号", "所属院系", "专业名称", "专业介绍" };
	JOptionPane jOptionPane1 = new JOptionPane();
	DefaultTableModel model = new DefaultTableModel();
	String sql, find;
	int intRow;

	public SpecialtyManageFrame() {
		try {
			init();
	...
	private void init() throws Exception {
		...
		sql = "select s.id, s.departmentId, s.cname, s.remark, d.cname AS departmentCname FROM specialty AS s LEFT JOIN department AS d ON s.departmentId = d.id WHERE s.isDel = false";
		// 刷新
		UpdateRecord();
	}
	...
	public void jButton4_actionPerformed(ActionEvent e) {
		...
		if (intRow == -1) {
			...
			SpecialtyChangeFrame siadd = new SpecialtyChangeFrame(find);
			siadd.setLocation(400, 200);
			siadd.setSize(465, 360);
	...
	public void jButton5_actionPerformed(ActionEvent e) {
		...
		try {
			conn.getUpdate("update specialty set isDel = true where id = '" + Integer.valueOf(find) + "'");
...
class DepartManager_jButton6_actionAdapter implements ActionListener {
	private SpecialtyManageFrame adaptee;

	DepartManager_jButton6_actionAdapter(SpecialtyManageFrame adaptee) {
...
class DepartManager_jButton5_actionAdapter implements ActionListener {
	...
	DepartManager_jButton5_actionAdapter(SpecialtyManageFrame adaptee) {
		this.adaptee = adaptee;
	}
...
class DepartManager_jButton4_actionAdapter implements ActionListener {
	...
	DepartManager_jButton4_actionAdapter(SpecialtyManageFrame adaptee) {
		this.adaptee = adaptee;
	}
